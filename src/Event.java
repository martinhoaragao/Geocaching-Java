...
public class Event {
     ...
     */
    public Event (Double id, GregorianCalendar registTime, ArrayList<Cache> caches) {
        this.id = id;
     ...
     * @arg currentUser, user on which we are doing the simulation
     */
    ...
    /**
     * Builds a string with the activities generated by an Event
     */
     public String activitiesInfo () throws NoStatsException {
        Activity activity;
        StringBuilder sb = new StringBuilder();
        int i = 0, regMonth, curMonth, regYear, curYear;
        regMonth = registTime.get(GregorianCalendar.YEAR);
        curMonth = regMonth;
        regYear = registTime.get(GregorianCalendar.MONTH);
        curYear = regYear;
        while(true) {
            Statistic stat = this.users.get(i).getStatistics().getStatistic(curYear);
            activity = stat.getMonthActivities(curMonth).pollLast();
            while(activity.getDate().compareTo(registTime) > 0)
                sb.append("Found cache: " + activity.getCache().getId() + "\n");
